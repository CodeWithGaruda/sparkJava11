...
public class RDDBroadcastTest implements Serializable {
    ...
    private final SparkConf sparkConf = new SparkConf().setAppName("RDDBroadcastTest").setMaster("local[*]");
    ...
    void testBroadcastInSparkRDD() {
        try (final var sparkContext = new JavaSparkContext(sparkConf)) {
            ...
            final Broadcast<Map<String, String>> broadcastTickerStockName = sparkContext.broadcast(tickerStockName);
            final Broadcast<Map<String, Double>> broadcastTickerLastClosePrice = sparkContext.broadcast(tickerLastClosePrice);
            try {
                ...
                final var lastClosePriceRdd = myRdd.map(ticker -> {
                    ...
                    return String.format("Ticker=%s, Full Stock Name=%s, Last Close Price=%.2f", ticker, tickerFullName, tickerClosePrice);
                });

package com.rayala.chapter07_rddmapping;
...
public class RDDFlatMapsTest {
    ...
    void testFlatMapInSparkRDD() {
        ...
        try (final var sc = new JavaSparkContext(conf)) {
            ...
            JavaRDD<String> lines = sc.textFile(testFilePath);
            System.out.printf("Total lines in file %d%n", lines.count());

            JavaRDD<List<String>> mappedLines = lines.map(line -> List.of(line.split("\\s")));
